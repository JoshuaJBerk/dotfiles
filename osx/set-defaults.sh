...
# Restore Launchpad + Wallpapers from Backup -- > Must manually rename first on a new build.
# Change picture every day, random order, from Google Drive/Photos/Wallpapers
# cp -r ~/GoogleDrive/System/4630BAA2-A68A-40E7-9F69-94EEA738C085.db ~/Library/Application\ Support/Dock/4630BAA2-A68A-40E7-9F69-94EEA738C085.db 2> /dev/null
# cp -r ~/GoogleDrive/System/desktoppicture.db ~/Library/Application\ Support/Dock/desktoppicture.db 2> /dev/null
...
# osascript -e 'tell application "System Events" to make new login item at end of login items with properties { path: "/Applications/uTorrent.app", hidden:true }' > /dev/null 2>&1
